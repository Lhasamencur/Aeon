Shader "AEON_BRDF_Texture" 
{
	Properties  
	{
		...
		_BRDFAmount ( "BRDF Amount", Range( 0.0, 1.0 ) ) = 0.125
		_BRDFRoughnessX ( "BRDF Roughness X", Range( 0.0, 2.0 ) ) = 1.0
		_BRDFRoughnessY ( "BRDF Roughness Y", Range( 0.0, 2.0 ) ) = 1.0
	...
	SubShader 
	{
		...
		{	
			...
			float diffuseDot = saturate( dot( lDir, norm ) );
			float fresnelDot = min( 1.0, dot( vDir, norm ) );
            float dotHTAlphaX = dot( halfVector, tangent ) / _BRDFRoughnessX;
            float dotHBAlphaY = dot( halfVector, cross( norm, tangent ) ) / _BRDFRoughnessY;
            
            float BRDF = sqrt( saturate( diffuseDot / fresnelDot ) ) 
                  * exp( -2.0 * ( dotHTAlphaX * dotHTAlphaX 
                  + dotHBAlphaY * dotHBAlphaY ) / ( 1.0 + specDot ) );
            ...
            float SpecularCore = lerp( BRDF * 0.75, doubleSpec, _BRDFAmount );
		...
		void surf( Input IN, inout SurfaceOutput o ) 
		{
			...
			o.Albedo =  IN.tangent; // Smuggle Tangent in Albedo;

			o.Normal = UnpackNormal( tex2D( _Normalmap, IN.uv_Normalmap ) );
			o.Gloss = 0.275 * tex; // Smuggle Albedo in Gloss;
